...
/**
  ...
  */
class NumExprNumeric(implicit model: MpModel) extends Numeric[NumExpr] {

  override def fromInt(x: Int): NumExpr = NumExpr(model.cplex.linearNumExpr(x))
...
object NumExprNumeric {
  ...
  def apply(model: MpModel): Numeric[NumExpr] = new NumExprNumeric()(implicitly(model))

  implicit def num(implicit model: MpModel): Numeric[NumExpr] = model.getNumExprNumeric()
}
